...
public class SavinPlanServiceImpl implements SavinPlanService {
    ...
    @Autowired
    public SavinPlanServiceImpl(SavinPlanRepository savinPlanRepository, ModelMapper modelMapper) {
    ...
    public SavinPlanDTO createSavinPlan(SavinPlanDTO savinPlanDTO) {
        if (savinPlanRepository.findById(savinPlanDTO.getIdPlan()).isPresent()) {
            throw new CustomException("Savin plan with this ID already exists");
        }
